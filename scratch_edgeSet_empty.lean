...
lemma SimpleGraph.IsRegularOfDegree'.edgeSet_empty {V : Type}
  ...
  intro hadj
  have hdeg := hreg v
  simp [SimpleGraph.IsRegularOfDegree, SimpleGraph.degree] at hdeg
  have : w ∈ (hlf.neighborFinset v) := by
    rwa [SimpleGraph.LocallyFinite.neighborFinset, Finset.mem_filter] at *
    · exact ⟨Finset.mem_univ w, hadj⟩
  rw [hdeg] at this
  exact Finset.not_mem_empty w this
